...
def app_base() -> Path:
    ...
    # if vault_core.py lives in /core, use the project root
    if here.name.lower() == "core":
        return here.parent
...
def _web_entry_path() -> Path:
    return bundle_base() / "core" / "visionvault_web.py"
...
def start_tv_mode(port: int = 5050) -> tuple[bool, str]:
    ...
    try:
        from core.visionvault_web import start_tv_server_thread

        ok, msg = start_tv_server_thread(host="0.0.0.0", port=port)
        if ok:
            _tv_running = True
...
def stop_tv_mode() -> tuple[bool, str]:
    ...
    try:
        from core.visionvault_web import stop_tv_server

        ok, msg = stop_tv_server()
        if ok:
            _tv_running = False
        ...
        return False, f"Could not stop TV Mode: {e}"
